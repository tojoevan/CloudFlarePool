...
1. **新业务逻辑尽量进数据湖（serverless）**：本地 CLI 部署天然零 SSH、免备案、全球边缘；VPS 网关只留**薄代理 + admin + deploy 触发**，保持**稳定少改**。
   - **契约归网关、实现归数据湖**（2026-09-06 补，详见 `11-能力演进规划` §〇）：对外契约由网关持有并**屏蔽数据湖**，业务只认网关语义化 API；数据湖的路由/字段/存储可自由演进，**不得传导为业务端改动**。反例：业务端直接拼 `/t/{tenant}/xxx` 这类数据湖路径（当前 `/api/data/*` 原样透传即属此类欠账）。
   - 推论：映射关系应**外置配置化**，改映射不重启网关。
