...
tenantsRoute.post('/', async (c) => {
  ...
  return c.json({ ok: true, tenant: b.tenant_id, app_id: b.app_id ?? DEFAULT_APP_ID });
});

// Read a tenant's metadata.
tenantsRoute.get('/:id', async (c) => {
  const row = await c.env.DB.prepare(
    `SELECT tenant_id, appid, app_id, name, plan, quota, created_at FROM tenants WHERE tenant_id = ?`
  )
    .bind(c.req.param('id'))
    .first();
